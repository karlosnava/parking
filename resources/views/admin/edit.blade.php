...
@section('content')
	<div class="w-9/12 mx-auto bg-white shadow-md p-8 my-10">
		...
		<div class="mb-8">
			<h4 class="text-3xl font-bold">Editar vehículo</h4>
		...
		<form action="{{ route('cars.update', $car->id) }}" method="POST">
			...
			<div class="grid grid-cols-2 gap-3">
				<div>
					<input class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker" type="text" name="plate" placeholder="Placa" required value="{{ old('plate', $car->plate) }}">

					@error('plate') <small class="text-red-500">{{ $message }}</small> @enderror
				</div>

				<div>
					<input class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker" type="text" name="phone" placeholder="Teléfono" required value="{{ old('phone', $car->phone) }}">
			...
			<div class="grid grid-cols-2 gap-3 my-5">
				<div>
					<input class="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker" type="text" name="color" placeholder="Color" required value="{{ old('color', $car->color) }}">
